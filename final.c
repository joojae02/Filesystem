	...
	if (access("myfs", F_OK) != -1)
	{
		...
		if (c == 'y')
		{
			...
			for (int i = 0; i < 128; i++) {
				...
				if ((inode + i)->name != NULL)
				{
					free((inode + i)->name);
					(inode + i)->name = NULL;	
				}
				if ((inode + i)->datablockInfo != NULL)
				{
	...
	if ( block < 8 || (block == 8 && block_left == 0))
	{
		...
		if (block_left != 0)
		{
			...
			*((inode + num)->datablockInfo + block) = num2;
			strncpy((datablock + num2)->data, (datablock + *((inode + tmp)->datablockInfo + block))->data, block_left);
		}
...
int mymkdir(char* f1)
{
	...
	*((inode + num1)->datablockInfo) = num2;
	...
	for(int i =0; i<16;i++)
		((datablock + num2)->directory + i)->name = NULL;
	(inode + in)->size += 8;
...
void myrm(char *f1)
{
	...
	else if(block > 8 || (block == 8 && block_left != 0))
	{
		...
		free((datablock + indir) -> indirect);
		(datablock + indir)->indirect = NULL;
	}
	free_inode(num1);
	for (int j = 0; j < 8; j++)
		*((inode + num1)->datablockInfo + j) = -1;
...
void setinode() {
	...
	inode = (struct inode*)malloc(sizeof(inode) * 1e9);
	for (int i = 0; i < 128; i++) {
		(inode + i)->name = (char*)malloc(sizeof(char) * 7);
		(inode + i)->datablockInfo = (int*)malloc(sizeof(int) * 16);
		for (int j = 0; j < 16; j++)
...
void setdatablock() {
	...
	datablock = (struct datablock*)malloc(sizeof(struct datablock) * 1e9);
	for (int i = 0; i < 256; i++)
	{
		(datablock + i)->data = NULL;
		(datablock + i)->directory = NULL;
	...
	for (int i = 0; i < 16; i++)
		(datablock->directory + i)->name = NULL;
}
void superblockprint() {
	printf("inode List : ");
	for (int i = 0; i < 4; i++)
